...
public class LookingForEachOther extends Fragment implements RecyclerViewAdapterMyMatches.OnLoadMoreListener, SwipeRefreshLayout.OnRefreshListener, dialogShowInterest.onCompleteListener, dialogRequestPhone.onCompleteListener, DashboardMatchesMainFragment.MatchesMainFragmentInterface, dialogRequest.onCompleteListener, dialogProfileCompletion.onCompleteListener, dialogRemoveFromSearch.onCompleteListener, UpdateMatchesCountCallback, MatchesRefreshCallBackInterface {
    ...
    private void initilize(View view) {
        ...
        ((AppCompatButton) view.findViewById(R.id.ButtonMyMatchesSubscribeClick)).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
    ...
    private void loadData(String paramsString, final boolean refresh) {
        ...
        final JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.PUT,
                        ...
                        try {
                            ...
                            if (jsonArray.length() > 1) {
                                ...
                                if (membersDataList.size() > 0) {
                                    llMMMatchesNotFound.setVisibility(View.GONE);
                                    llMMMatchesNotFoundCompleteProfile.setVisibility(View.GONE);
                                    ...
                                    if (SharedPreferenceManager.getUserObject(context).get_member_status() >= 0 && SharedPreferenceManager.getUserObject(context).get_member_status() <= 2) {
                                        ...
                                        llMMMatchesNotFound.setVisibility(View.VISIBLE);
                                    }
                                }
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                pDialog.setVisibility(View.GONE);
                // }
                if (SharedPreferenceManager.getUserObject(context).get_member_status() == 1) {
                    llMMMatchesNotFoundCompleteProfile.setVisibility(View.VISIBLE);
